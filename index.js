...
function bounceBall() {
    ...
    let [bool,idx] = isBlockCollision(allBlocks);
    if (bool) {
        directionAtBlockCollision(allBlocks[idx]);
        removeBlock(idx);
    }
    checkGameOver();
...
//------------------------------------------------------------------------
// CHANGE DIRECTION AT BLOCK COLLISION
function directionAtBlockCollision(block) { // block is allBlocks[i]
    if (ball.arrMotion[0] == 1 && ball.arrMotion[1] == 1) {// DIRECTION [1,1]
        if (ball.y == block.bottomLeft[1]) {
    ...
    } else if (ball.arrMotion[0] == -1 && ball.arrMotion[1] == 1) {// DIRECTION [-1,1]
        if (ball.y == block.bottomLeft[1]) {
...
function checkBorderCollision() {
    ...
    if (ball.arrMotion[0] == -1 && ball.arrMotion[1] == -1) {// DIRECTION [-2,-2]
        if (// MEET THE LEFT
            ...
            ball.moveFourthQuadrant();
            ball.move();
        } else if (// COLLISION AT PLAYER-BALL-BORDER CONNECTION
            ball.x == 0 && player.x == 0 && (player.y + 20 >= ball.y)
        ) {
            ball.moveFirstQuadrant();
            ball.move();
        }
    ...
    if (ball.arrMotion[0] == 1 && ball.arrMotion[1] == -1) {// DIRECTION [2,-2]
        if (// MEET THE RIGHT BORDER
            ...
            (ball.x == gridDimension[0] - ballDimension) &&
            (player.x == gridDimension[0] - ballDimension) &&
            (player.y + 20 >= ball.y)
        ) {
            ball.moveThirdQuadrant();
            ball.move();
